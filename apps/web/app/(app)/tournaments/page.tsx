...
// Demo bracket data with card names and records
const BRACKET_DATA = {
  qf: [
    ...
    { left: 'slabmaster', right: 'gradegod', leftCard: 'Allen \'18 Prizm Auto', rightCard: 'LeBron \'03 Chrome', leftRecord: '2-1', rightRecord: '2-1', winner: undefined as string | undefined },
    ...
    { left: 'TBD', right: 'TBD', leftCard: 'TBD', rightCard: 'TBD', leftRecord: '-', rightRecord: '-', winner: undefined as string | undefined },
  ],
  sf: [
    { left: 'cardking', right: 'TBD', leftCard: 'Mahomes \'17 Prizm', rightCard: 'TBD', winner: undefined as string | undefined },
    { left: 'rookiehunter', right: 'TBD', leftCard: 'Wemby \'23 Prizm Auto', rightCard: 'TBD', winner: undefined as string | undefined },
  ],
  final: { left: 'TBD', right: 'TBD', leftCard: 'TBD', rightCard: 'TBD', winner: undefined as string | undefined },
};
...
function BracketSlot({ username, cardName, record, winner, isTBD }: MatchupSlot) {
  return (
    <div
      className="px-3 py-2 flex items-center gap-2"
      style={{ background: winner ? 'rgba(108,71,255,0.12)' : 'transparent' }}
    >
      {winner && <div className="w-1.5 h-1.5 rounded-full bg-[#6c47ff] flex-shrink-0" />}
      <div className="min-w-0 flex-1">
        <p
          className="text-xs font-bold truncate"
          style={{ color: isTBD ? '#374151' : winner ? '#a78bfa' : '#94a3b8' }}
        >
          {isTBD ? 'TBD' : username}
        </p>
        {!isTBD && (
          <p className="text-[9px] text-[#475569] truncate">{cardName}</p>
        )}
      </div>
      {record && !isTBD && (
        <span className="text-[9px] text-[#475569] flex-shrink-0">{record}</span>
      )}
    </div>
  );
}
...
function BracketMatchCard({ round, matchup, isActive }: { round: string; matchup: MatchupData; isActive?: boolean }) {
  const isTBDLeft = matchup.left === 'TBD';
  const isTBDRight = matchup.right === 'TBD';
  ...
  return (
    <div
      ...
      style={{
        background: '#12121a',
        borderColor: isActive ? 'rgba(108,71,255,0.5)' : '#1e1e2e',
        boxShadow: isActive ? '0 0 12px rgba(108,71,255,0.2)' : 'none',
        minWidth: '160px',
      }}
    >
      ...
      <div className="divide-y divide-[#1e1e2e]">
        <BracketSlot
          username={matchup.left}
          cardName={matchup.leftCard}
          record={matchup.leftRecord}
          winner={matchup.winner === matchup.left && !isTBDLeft}
          isTBD={isTBDLeft}
        />
        <BracketSlot
          username={matchup.right}
          cardName={matchup.rightCard}
          record={matchup.rightRecord}
          winner={matchup.winner === matchup.right && !isTBDRight}
          isTBD={isTBDRight}
        />
      </div>
    </div>
...
function BracketView({ tournament }: { tournament: Tournament & { type?: string } }) {
    ...
    <div className="overflow-x-auto pb-4">
      <div className="flex items-start gap-4 min-w-[580px] py-4">
        ...
        <div className="flex flex-col gap-3">
          ...
          {BRACKET_DATA.qf.map((m, i) => (
            <BracketMatchCard key={i} round="QF" matchup={m} isActive={!!m.winner} />
          ))}
        </div>
        ...
        <div className="flex flex-col justify-around pt-8" style={{ height: 370 }}>
          {[0, 1].map((i) => (
            <div key={i} className="flex items-center" style={{ height: 80 }}>
        ...
        <div className="flex flex-col gap-20 pt-4">
          ...
          {BRACKET_DATA.sf.map((m, i) => (
            <BracketMatchCard key={i} round="SF" matchup={m} />
          ))}
        </div>

        {/* Connector SF→Final */}
        <div className="flex items-center" style={{ marginTop: 90 }}>
          <div className="w-4 border-t-2 border-[#1e1e2e]" />
          <ChevronRight size={12} className="text-[#374151]" />
        </div>
...
function EnterTournamentModal({ tournamentId, onClose }: { tournamentId: string; onClose: () => void }) {
  ...
  const SAMPLE_CARDS = [
    'Patrick Mahomes 2017 Prizm RC PSA 10',
    'Tom Brady 2000 Bowman Chrome RC',
    'LeBron James 2003 Topps Chrome RC',
    'Victor Wembanyama 2023 Prizm Auto',
    'Luka Doncic 2018 Prizm RC',
    'Josh Allen 2018 Prizm Rookie Auto',
    'Shohei Ohtani 2018 Topps RC',
    'Caitlin Clark 2024 Parkside Auto',
  ];

  const filtered = cardSearch
    ? SAMPLE_CARDS.filter(c => c.toLowerCase().includes(cardSearch.toLowerCase()))
    : SAMPLE_CARDS;
  ...
  const handleEnter = async () => {
    ...
    await fetch(`${API}/tournaments/${tournamentId}/enter`, {
      method: 'POST',
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-[#1e1e2e]">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-white">Enter Tournament</h3>
          </div>
        ...
        {entered ? (
          <div className="p-8 text-center space-y-3">
            ...
            <p className="text-sm text-[#94a3b8]">
              <span className="text-[#a78bfa] font-semibold">{selectedCard}</span> has been registered.
            </p>
            <p className="text-xs text-[#475569]">Good luck in the tournament!</p>
            <button onClick={onClose} className="mt-2 w-full py-2.5 rounded-xl bg-[#6c47ff] text-white text-sm font-bold">
              Close
            </button>
          </div>
        ) : (
          <div className="p-4 space-y-3">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-[#0a0a0f] border border-[#1e1e2e] rounded-xl pl-8 pr-3 py-2 text-sm text-white placeholder:text-[#374151] focus:outline-none focus:border-[#6c47ff]"
              />
            </div>
              ...
              {filtered.map(card => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    {selectedCard === card && <Check size={13} className="flex-shrink-0 ml-2" />}
                  </div>
                </button>
              ))}
            </div>
      ...
      {participants.map((p, i) => (
        <div key={p.username} className="flex items-center gap-3 p-3 rounded-xl border border-[#1e1e2e]" style={{ background: '#0a0a0f' }}>
          <div
            ...
            style={{ background: i === 0 ? 'rgba(251,191,36,0.2)' : 'rgba(108,71,255,0.15)', color: i === 0 ? '#fbbf24' : '#a78bfa' }}
          >
            {p.seeded}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-xs font-bold text-white">@{p.username}</p>
            <p className="text-[10px] text-[#475569] truncate">{p.cardName}</p>
          </div>
...
function TournamentCard({ tournament, onSelect, isSelected }: {
  tournament: Tournament & { type?: string };
  onSelect: () => void;
  isSelected: boolean;
}) {
  ...
  return (
    <button
      ...
      style={{ background: isSelected ? 'rgba(108,71,255,0.08)' : '#12121a', borderColor: isSelected ? 'rgba(108,71,255,0.5)' : '#1e1e2e' }}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <p className="font-bold text-white text-sm truncate">{tournament.name}</p>
          <p className="text-[11px] text-[#64748b] mt-0.5">{new Date(tournament.createdAt).toLocaleDateString()}</p>
        </div>
      ...
      <div className="mt-2">
        <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: typeBadge.bg, color: typeBadge.text }}>
          {typeBadge.label}
        </span>
      </div>
    </button>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={onClose}>
      <div className="w-full max-w-sm rounded-2xl border border-[#1e1e2e] overflow-hidden" style={{ background: '#12121a' }} onClick={e => e.stopPropagation()}>
        ...
        <div className="p-4 space-y-3">
          <div>
            <label className="text-xs font-bold text-[#94a3b8] uppercase tracking-wider block mb-1.5">Name</label>
            <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. NBA GOAT Bracket 2026" className="w-full bg-[#0a0a0f] border border-[#1e1e2e] rounded-xl px-3 py-2.5 text-sm text-white placeholder-[#374151] focus:outline-none focus:border-[#6c47ff]/50" />
          </div>
          <div>
            <label className="text-xs font-bold text-[#94a3b8] uppercase tracking-wider block mb-1.5">Format</label>
            <div className="flex gap-2 flex-wrap">
              {Object.entries(TYPE_BADGES).map(([key, info]) => (
                <button key={key} onClick={() => setType(key)} className="px-2 py-1 rounded-lg text-[10px] font-bold transition-all border" style={type === key ? { background: info.bg, color: info.text, borderColor: info.text + '60' } : { background: 'transparent', color: '#64748b', borderColor: '#1e1e2e' }}>
                  {info.label}
                </button>
              ))}
            </div>
          ...
          <div>
            <label className="text-xs font-bold text-[#94a3b8] uppercase tracking-wider block mb-1.5">Sport</label>
            <div className="flex gap-2">
              {(['nfl', 'nba', 'mlb', 'mixed'] as const).map(s => {
                const info = SPORT_COLORS[s];
                return <button key={s} onClick={() => setSport(s)} className="flex-1 py-2 rounded-xl text-xs font-bold transition-all border" style={sport === s ? { background: info.bg, color: info.text, borderColor: info.text + '40' } : { background: 'transparent', color: '#64748b', borderColor: '#1e1e2e' }}>{info.label}</button>;
              })}
            </div>
          </div>
...
export default function TournamentsPage() {
  ...
  const typedTournaments = tournaments.map((t, i) => ({
    ...t,
    type: (['single_elimination', 'round_robin', 'lightning'] as const)[i % 3],
  }));
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        ...
        style={{ background: 'linear-gradient(135deg, #0f0721 0%, #12121a 40%, #0a0a0f 100%)', border: '1px solid rgba(108,71,255,0.2)' }}
      >
        <h1 className="text-2xl font-black text-white mb-1 flex items-center justify-center gap-2">
          <Trophy size={24} className="text-[#fbbf24]" /> Tournaments
        </h1>
          ...
          <div className="space-y-2">
            ...
            {typedTournaments.map(t => (
              <TournamentCard
                key={t.id}
                tournament={t}
                onSelect={() => { setSelectedId(t.id); setActiveTab('bracket'); }}
                isSelected={selectedId ? selectedId === t.id : t === typedTournaments[0]}
              />
            ))}
          </div>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-1">
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: (SPORT_COLORS[typedSelected.sport] || SPORT_COLORS.mixed).bg, color: (SPORT_COLORS[typedSelected.sport] || SPORT_COLORS.mixed).text }}>
                        {(SPORT_COLORS[typedSelected.sport] || SPORT_COLORS.mixed).label}
                      </span>
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: (STATUS_STYLES[typedSelected.status] || STATUS_STYLES.open).bg, color: (STATUS_STYLES[typedSelected.status] || STATUS_STYLES